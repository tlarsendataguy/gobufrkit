...
func (r *PeekableBitReader) PeekUint(skip int, n int) (uint, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("cannot peek bytes: %w", err)
	}
	v, err := NewBitReader(bytes.NewReader(b)).ReadUint(n)
	if err != nil {
		return 0, fmt.Errorf("cannot read peeked bytes: %w", err)
	}
	return v, nil
}
...
func (r *PeekableBitReader) PeekBytes(skip int, nbytes int) ([]byte, error) {
	b, err := r.peek(skip, nbytes)
	if err != nil {
		return []byte{}, fmt.Errorf("cannot peek bytes: %w", err)
	}
	v, err := NewBitReader(bytes.NewReader(b)).ReadBytes(nbytes)
	if err != nil {
		return []byte{}, fmt.Errorf("cannot read peeked bytes: %w", err)
	}
	return v, nil
}
